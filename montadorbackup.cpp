...
//Vitor pls
int tabSimSeek0 (list <MacroNameTable> MNT, string token, int *nargumentos, int *linhamdt, int *linhamdtfim, string *argumento1, string *argumento2, string *argumento3, string *argumento4) {
    ...
    while (1) {
        if (it == MNT.end()) {
            ...
            *argumento1 = it-> macroarg[0];
            *argumento2 = it-> macroarg[1];
            *argumento3 = it-> macroarg[2];
            *argumento4 = it-> macroarg[3];
        }
    }
}
...
void macroProc (list <MacroNameTable> *MNT, list <int> nLinhasIN, list <int> nLinhasOUT, string nomeIN, string nomeOUT){ //Um tanto desses inteiros eu copiei da sua parte e tenho medo de apagar
    ...
    while (getline(arquivo,linha)) {
        ...
        while(linhaStream >> token){
            //errotoken
            if((tabSimSeek0(*MNT,token,&nargumentos,&mdtsearch,&mdtfim,&argumentodeclarado[0],&argumentodeclarado[1],&argumentodeclarado[2],&argumentodeclarado[3]) > -1)){
                tirarlinha = 1;
                while(linhaStream >> token){
                    contarg++;
                    if (contarg == 1){
            ...
            if((iequals(token,"MACRO") == 1)){//Verifica se o token e um EQU, se for EQU ele manda o label anterior pra tabela
                ...
                if ((tokenaux.back()) == ':') {//Verifica se o token anterior a macro e um label
                    tokenaux.pop_back();
                    simEndereco = tabSimSeek0(*MNT, tokenaux,&nargumentos,&mdtsearch,&mdtfim,&argumentodeclarado[0],&argumentodeclarado[1],&argumentodeclarado[2],&argumentodeclarado[3]);
                    if (simEndereco > 0) {
